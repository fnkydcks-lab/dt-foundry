...
4. Read `dt-foundry/taste.md` if it exists — note what was flagged in the previous run and whether it was addressed. If the same issue appears again it's a persistent problem, not a new finding. If it's gone, note the improvement. Carry forward any **accepted / deliberate** decisions without re-litigating them.
5. Evaluate against these criteria in sequence:
...
6. Identify the one thing that doesn't feel right yet. Name it precisely. Do not name anything already flagged in preflight.
7. Run `mkdir -p dt-foundry` to ensure the output directory exists in the project root.
8. Write output to `dt-foundry/taste.md` with timestamp.
9. Print to terminal: what's working, what's not, the one thing to fix first.
